...
function SocialIcon({ path }: { path: string }) {
  return (
    <a
      href="#"
      className="flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-slate-300 transition hover:border-cyan-400 hover:text-cyan-400"
  ...
  return (
    <footer id="contact" className="bg-navy-950">
      <div className="mx-auto max-w-7xl px-6 py-14 text-center lg:px-10">
        ...
        <nav className="mt-8 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 text-sm text-slate-300">
          {links.map((l) => (
            <a key={l} href="#" className="transition hover:text-cyan-400">
              {l}
            </a>
          ))}
        </nav>

        <div className="mx-auto mt-8 h-px w-full max-w-md bg-white/10" />
        <p className="mt-6 text-xs text-slate-500">© 2026 SynCode. All rights reserved.</p>
      </div>
    </footer>
  );
}
