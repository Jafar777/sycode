...
export default function TechStack() {
  return (
    <section id="stack" className="border-t border-line bg-ink-900 py-20">
      <div className="mx-auto max-w-7xl px-6 text-center lg:px-10">
        <p className="font-mono text-[11px] uppercase tracking-widest2 text-steel-500">04 — Toolkit</p>
        <h2 className="mt-3 font-display text-2xl font-bold uppercase tracking-wide text-white sm:text-3xl">
          Technology Stack
        </h2>
        <p className="mt-3 text-steel-500">The tools behind everything we ship</p>

        <div className="mt-12 grid grid-cols-2 gap-8 sm:grid-cols-4">
          {stack.map((t) => (
            <div key={t.name} className="flex flex-col items-center gap-3">
              <span className="text-steel-300">{t.icon}</span>
              <span className="font-mono text-xs uppercase tracking-widest2 text-steel-400">{t.name}</span>
            </div>
          ))}
        </div>
      </div>
